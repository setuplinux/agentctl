...
- Multica: Linux, macOS, and Windows install paths download the latest CLI archive from `multica-ai/multica` GitHub releases; updates use the native `multica update` command. Run `multica setup` after install to authenticate and start the local daemon.
